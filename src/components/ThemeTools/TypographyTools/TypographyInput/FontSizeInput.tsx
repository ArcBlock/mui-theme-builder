...
function FontSizeInput({ value, onChange, property }: Props) {
  ...
  const [displayValue, setDisplayValue] = useState<number>(getFontSizeValue(value, fontSizeUnit));
    ...
    <>
        ...
        <Grid item>
          ...
          <Select
            ...
            onChange={(event) => {
              ...
              onChange(event, event.target.value === 'px' ? displayValue : `${displayValue}${event.target.value}`);
            }}
            margin="dense">
      ...
      <Slider
        ...
        onChangeCommitted={(event, newValue) => {
          ...
          onChange(event, fontSizeUnit === 'px' ? newVal : `${newVal}${fontSizeUnit}`);
        }}
      />
      {disabled && (
